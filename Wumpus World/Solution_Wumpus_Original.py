...
def ask_around(name):
    return updatewumpus.look_ahead(name)
...
def shoot_wumpus(name,wumpus_location,now_location):
    ...
    updatewumpus.take_action(name, orientation)  
    updatewumpus.take_action(name, 'Shoot')     
                     
def infer_pit(knowledge,now_location,around):
...
def infer_gold(name,knowledge,now_location):
    ...
    if('glitter' in knowledge[now_location]):
        updatewumpus.take_action(name, 'PickUp')
        get_gold=1
    return get_gold
     
def move_to_next(name, orientation):
    updatewumpus.take_action(name, orientation)
    return updatewumpus.take_action(name, 'Step')
...
def main():
    ...
    orientation = "Up"
    situation = updatewumpus.take_action(name, orientation)
